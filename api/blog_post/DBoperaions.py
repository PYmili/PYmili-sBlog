...
class Operations:
    ...
    def create(
        ...
        """
        insert_query = """
            INSERT INTO blog_posts (author, title, excerpt, url, image, text)
            VALUES (?, ?, ?, ?, ?, ?);
        """
        self.cursor.execute(insert_query, (author, title, excerpt, url, image, text))
        self.SqliteConnect.commit()
    ...
    def get_by_author(self, author: str) -> List[Dict[str, Union[int, str, bytes]]]:
        ...
        :return:
            List[Dict]: 包含博客文章记录信息的字典列表，每个字典包含：
              'id', 'author', 'title', 'excerpt', 'url', 'image' 和 'upload_time' 字段。
        """
        select_query = """
    ...
    def update_by_id_and_author(
        ...
        update_query = """
            ...
            WHERE id = ? AND author = ?;
        """
        params = [title, excerpt, url, image, post_id, author]
